! ( function () {
			...
			721: function () {
				function t( t ) {
					...
					console.log( n ),
						...
						n.setAttribute( 'data-bs-popper', 'static' );
				}
				document
					.querySelector( '.navbar' )
					.querySelectorAll( '.dropdown-toggle' )
					.forEach( ( e ) => {
						e.closest( '.mobile-navbar' ) ||
	...
	( i.d = function ( t, e ) {
		...
		( function () {
			...
			function L( t ) {
				return (
					t instanceof x( t ).HTMLElement || t instanceof HTMLElement
				);
			}
			function S( t ) {
				return (
			...
			function H( t, e, i ) {
				...
				e &&
					L( t ) &&
					( ( s =
						( t.offsetWidth > 0 && P( n.width ) / t.offsetWidth ) ||
						1 ),
					( o =
						( t.offsetHeight > 0 &&
							P( n.height ) / t.offsetHeight ) ||
						1 ) );
			...
			function z( t ) {
				return (
					( k( t ) ? t.ownerDocument : t.document ) || window.document
				).documentElement;
			}
			function R( t ) {
				return 'html' === O( t )
			...
			function K( t ) {
				for (
					var e = x( t ), i = V( t );
					i && q( i ) && 'static' === B( i ).position;

				)
					i = V( i );
				return i &&
					( 'html' === O( i ) ||
						( 'body' === O( i ) && 'static' === B( i ).position ) )
					? e
					: i ||
							( function ( t ) {
			...
			function ct( t ) {
				return H( z( t ) ).left + lt( t ).scrollLeft;
			}
			function ht( t ) {
				var e = B( t ),
					i = e.overflow,
			...
			function gt( t ) {
				...
				switch ( u ) {
					case e:
						i = { x: f, y: r.y - a.height };
						break;
					case n:
						i = { x: f, y: r.y + r.height };
						break;
					case s:
						i = { x: r.x + r.width, y: p };
						break;
			...
			function mt( t, i ) {
				...
				var o = i,
					...
					$ = ( function ( t, e, i, n ) {
						...
						k( D ) ? D : D.contextElement || z( t.elements.popper ),
						m,
						b,
						p
					),
					...
					q = {
						top: $.top - W.top + C.top,
						bottom: W.bottom - $.bottom + C.bottom,
						left: $.left - W.left + C.left,
						right: W.right - $.right + C.right,
					},
					V = t.modifiersData.offset;
				if ( y === d && V ) {
					var Q = V[ l ];
					Object.keys( q ).forEach( function ( t ) {
						var i = [ s, n ].indexOf( t ) >= 0 ? 1 : -1,
							o = [ e, n ].indexOf( t ) >= 0 ? 'y' : 'x';
						q[ t ] += Q[ o ] * i;
					} );
				}
				return q;
			...
			function vt( t ) {
				return [ e, s, n, o ].some( function ( e ) {
					return t[ e ] >= 0;
				} );
			}
			var yt = {
				...
				wt = {
					...
					fn: function ( t ) {
						...
						null != i.modifiersData.popperOffsets &&
							( ( i.modifiersData.popperOffsets.x += u ),
							( i.modifiersData.popperOffsets.y += d ) ),
							( i.modifiersData[ r ] = c );
					},
				},
				At = {
				...
				Et = {
					...
					fn: function ( t ) {
						var i = t.state,
							...
							C = Q( A ),
							O = 'x' === C ? 'y' : 'x',
							x = i.modifiersData.popperOffsets,
							k = i.rects.reference,
							L = i.rects.popper,
							S =
								'function' == typeof y
								...
								var j,
									...
									Z =
										b && J
											? W( J )
											: { width: 0, height: 0 },
									tt = i.modifiersData[ 'arrow#persistent' ]
										? i.modifiersData[ 'arrow#persistent' ]
												.padding
										: {
												top: 0,
												right: 0,
					...
					var s,
						...
						h = {
							...
							setOptions: function ( i ) {
								...
								return (
									( a.orderedModifiers = d.filter( function (
										t
									) {
										return t.enabled;
									} ) ),
			...
			var Lt = kt(),
				St = kt( {
					defaultModifiers: [ nt, At, et, D, wt, _t, Et, J, yt ],
				} ),
			...
			const be = {
				...
				getDataAttribute( t, e ) {
					return me( t.getAttribute( `data-bs-${ _e( e ) }` ) );
				},
			};
			class ve {
				static get Default() {
				...
				_mergeConfigObj( t, e ) {
					const i = jt( e ) ? be.getDataAttribute( e, 'config' ) : {};
					return {
						...this.constructor.Default,
						...( 'object' == typeof i ? i : {} ),
						...( jt( e ) ? be.getDataAttributes( e ) : {} ),
						...( 'object' == typeof t ? t : {} ),
					};
				}
			...
			class Ce extends ye {
				...
				close() {
					if ( de.trigger( this._element, Ee ).defaultPrevented )
						return;
			...
			class je extends ve {
				...
				_initEvents() {
					this._supportPointerEvents
						? ( de.on( this._element, Ie, ( t ) =>
								this._start( t )
						  ),
						  de.on( this._element, Ne, ( t ) => this._end( t ) ),
						  this._element.classList.add( 'pointer-event' ) )
						: ( de.on( this._element, Se, ( t ) =>
								this._start( t )
						  ),
						  de.on( this._element, De, ( t ) => this._move( t ) ),
						  de.on( this._element, $e, ( t ) => this._end( t ) ) );
				}
			...
			class ri extends ye {
				...
				pause() {
					this._isSliding && Mt( this._element ),
						this._clearInterval();
				}
				cycle() {
					this._clearInterval(),
				...
				to( t ) {
					...
					if ( this._isSliding )
						return void de.one( this._element, Ve, () =>
							this.to( t )
						);
				...
				_slide( t, e = null ) {
					...
					const i = this._getActive(),
						...
						s =
							e ||
							Ut( this._getItems(), i, n, this._config.wrap );
					if ( s === i ) return;
				...
				_directionToOrder( t ) {
					return Kt() ? ( t === qe ? Be : Fe ) : t === qe ? Fe : Be;
				}
				_orderToDirection( t ) {
					return Kt() ? ( t === Be ? qe : ze ) : t === Be ? ze : qe;
				}
				static jQueryInterface( t ) {
					return this.each( function () {
			...
			class yi extends ye {
					...
					if (
						( this._config.parent &&
							( t = this._getFirstLevelChildren(
								...
								.filter( ( t ) => t !== this._element )
								.map( ( t ) =>
									yi.getOrCreateInstance( t, { toggle: ! 1 } )
								) ),
						t.length && t[ 0 ]._isTransitioning )
					)
						return;
					if ( de.trigger( this._element, li ).defaultPrevented )
						return;
				...
				hide() {
					if ( this._isTransitioning || ! this._isShown() ) return;
					if ( de.trigger( this._element, hi ).defaultPrevented )
						return;
			...
			class Vi extends ye {
				constructor( t, e ) {
					super( t, e ),
						...
						( this._menu =
							ke.next( this._element, Mi )[ 0 ] ||
							ke.prev( this._element, Mi )[ 0 ] ||
							ke.findOne( Mi, this._parent ) ),
				...
				show() {
					...
					if (
						! de.trigger( this._element, ki, t ).defaultPrevented
					) {
						if (
							( this._createPopper(),
							'ontouchstart' in document.documentElement &&
				...
				_completeHide( t ) {
					if (
						! de.trigger( this._element, Oi, t ).defaultPrevented
					) {
						if ( 'ontouchstart' in document.documentElement )
							for ( const t of [].concat(
								...document.body.children
							) )
								de.off( t, 'mouseover', qt );
			...
			de.on( document, Di, Ni, Vi.dataApiKeydownHandler ),
				de.on( document, Di, Mi, Vi.dataApiKeydownHandler ),
				de.on( document, Si, Vi.clearMenus ),
				de.on( document, $i, Vi.clearMenus ),
				de.on( document, Si, Ni, function ( t ) {
			...
			class hn extends ve {
				...
				deactivate() {
					this._isActive &&
						( ( this._isActive = ! 1 ), de.off( document, sn ) );
				}
				_handleFocusin( t ) {
			...
			class kn extends ye {
				...
				show( t ) {
					this._isShown ||
						...
						de.trigger( this._element, gn, { relatedTarget: t } )
							.defaultPrevented ||
						( ( this._isShown = ! 0 ),
						( this._isTransitioning = ! 0 ),
				...
				_triggerBackdropTransition() {
					if ( de.trigger( this._element, fn ).defaultPrevented )
						return;
					const t =
			...
			class Kn extends ye {
				...
				hide() {
					this._isShown &&
						( de.trigger( this._element, Hn ).defaultPrevented ||
							( this._focustrap.deactivate(),
							this._element.blur(),
							( this._isShown = ! 1 ),
			...
			de.on(
				...
				function ( t ) {
					...
					const i = ke.findOne( Pn );
					i && i !== e && Kn.getInstance( i ).hide(),
						Kn.getOrCreateInstance( e ).toggle( this );
				}
			),
				de.on( window, Dn, () => {
					for ( const t of ke.find( Pn ) )
			...
			const ms = '.bs.scrollspy',
				...
				ws = '[href]',
				As = '.nav-link',
				Es = `${ As }, .nav-item > ${ As }, .list-group-item`,
				Ts = {
					offset: null,
			...
			class Os extends ye {
				...
				_maybeEnableSmoothScroll() {
					this._config.smoothScroll &&
						( de.off( this._config.target, bs ),
						de.on( this._config.target, bs, ws, ( t ) => {
				...
				_initializeTargetsAndObservables() {
					( this._targetLinks = new Map() ),
						( this._observableSections = new Map() );
					const t = ke.find( ws, this._config.target );
					for ( const e of t ) {
				...
				_process( t ) {
					this._activeTarget !== t &&
						...
						de.trigger( this._element, _s, { relatedTarget: t } ) );
				}
				_activateParents( t ) {
					if ( t.classList.contains( 'dropdown-item' ) )
						...
						for ( const e of ke.parents( t, '.nav, .list-group' ) )
							for ( const t of ke.prev( e, Es ) )
								t.classList.add( ys );
				}
				_clearActiveClass( t ) {
					t.classList.remove( ys );
					const e = ke.find( `${ ws }.${ ys }`, t );
					for ( const t of e ) t.classList.remove( ys );
				}
				static jQueryInterface( t ) {
					return this.each( function () {
			...
			const xs = '.bs.tab',
				...
				Rs = `.nav-link${ qs }, .list-group-item${ qs }, [role="tab"]${ qs }, ${ zs }`,
				Vs = `.${ Ws }[data-bs-toggle="tab"], .${ Ws }[data-bs-toggle="pill"], .${ Ws }[data-bs-toggle="list"]`;
			class Ks extends ye {
				constructor( t ) {
					super( t ),
				...
				show() {
					...
					const e = this._getActiveElem(),
						i = e
							? de.trigger( e, ks, { relatedTarget: t } )
							: null;
					de.trigger( t, Ss, { relatedTarget: e } )
						.defaultPrevented ||
				...
				_keydown( t ) {
					if ( ! [ Ps, Ms, js, Hs ].includes( t.key ) ) return;
					t.stopPropagation(), t.preventDefault();
					const e = [ Ms, Hs ].includes( t.key ),
						i = Ut(
				...
				_getChildren() {
					return ke.find( Rs, this._parent );
				}
				_getActiveElem() {
					return (
				...
				_getInnerElement( t ) {
					return t.matches( Rs ) ? t : ke.findOne( Rs, t );
				}
				_getOuterElement( t ) {
			...
			de.on( document, $s, zs, function ( t ) {
				[ 'A', 'AREA' ].includes( this.tagName ) && t.preventDefault(),
					Ft( this ) || Ks.getOrCreateInstance( this ).show();
			} ),
				de.on( window, Ns, () => {
					for ( const t of ke.find( Vs ) )
			...
			class ao extends ye {
				...
				_setListeners() {
					de.on( this._element, Xs, ( t ) =>
						this._onInteraction( t, ! 0 )
					),
						de.on( this._element, Ys, ( t ) =>
							this._onInteraction( t, ! 1 )
						),
						de.on( this._element, Us, ( t ) =>
							this._onInteraction( t, ! 0 )
						),
						de.on( this._element, Js, ( t ) =>
							this._onInteraction( t, ! 1 )
						);
				}
				_clearTimeout() {
			...
			var lo = new ( class {
					getSchedule() {
						const t = localStorage.getItem( 'schedule' ),
					...
					addToSchedule( t ) {
						...
						return new Promise( ( t, i ) => {
							try {
								this.checkTargetElement( e );
								const n = Number( e.dataset.id ),
									s = this.getSchedule();
								try {
									this.getEventData( n ).then( ( e ) => {
										const i = e.day.toLowerCase(),
											n = s[ i ].filter(
												( t ) => t.id === e.id
											);
										0 === n.length
					...
					checkTargetElement( t ) {
						if ( ! t )
							throw new Error( 'No target element provided' );
						if ( 'false' === t.dataset.addToSchedule )
							throw new Error(
								"This button doesn't control scheduling!"
							);
						if ( void 0 === t.dataset.id )
							throw new Error(
								`id or route is undefined! \n id: ${ t.dataset.id } `
							);
					}
					getEventData = async ( t ) => {
						try {
							const e = await fetch(
									`${ cnoSiteData.rootUrl }/wp-json/wp/v2/events/${ t }?_fields=acf,title,link`
								),
								i = await e.json(),
								{
									acf: { info: n },
				...
				co = new ( class {
							...
							this.buttons.forEach( ( e ) => {
								e.addEventListener( 'click', ( e ) => {
									...
									n &&
										t( e )
											.then( ( t ) => {
												( n.innerHTML = `<div class='alert alert-${ t }' role='alert'>${ this.getResponseMessage(
													t
												) }</div>` ),
													setTimeout( () => {
														n.innerHTML = '';
					...
					getResponseMessage( t ) {
						return 'success' === t
							...
							: 'info' === t
							? 'This is already in your schedule.'
							: '';
					}
				} )();
			new ( class {
				...
				constructor() {
					co.buttons.length > 0 &&
						co.clickHandler( lo.addToSchedule.bind( lo ) ),
						this.debug && this.#t();
				}
				#t() {
					console.log( 'hello from schedule-handler' ),
						console.log( co.buttons );
				}
			} )(),
				i( 721 );
		} )();
