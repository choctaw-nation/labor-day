! ( function () {
	...
	function t() {
		return ( 0, e.createElement )(
			'div',
			{ className: 'lds-ring' },
			[ 1, 2, 3, 4 ].map( ( t ) => ( 0, e.createElement )( 'div', null ) )
		);
	}
	const { postsPerPage: n, rootUrl: s } = cnoSiteData,
		r = `${ s }/graphql`,
		i = {
			keys: [
				{ name: 'title', weight: 1 },
	...
	var c = new ( class {
		async makeRequest( e ) {
			try {
				...
				throw ( console.error( 'makeRequest error:', e ), e );
			}
		}
		async getPosts() {
			const e = {
				query: 'query Events($first: Int = 4, $after: String = "", $include: [MediaItemSizeEnum] = [LARGE], $size: MediaItemSizeEnum = LARGE) {\n  events(after: $after, first: $first) {\n    pageInfo {\n      hasNextPage\n      endCursor\n    }\n    nodes {\n      eventLocations {\n        nodes {\n          name\n          link\n          event_locationId\n        }\n      }\n      event_info {\n        description\n        info {\n          day\n          endTime\n          startTime\n        }\n      }\n      featuredImage {\n        node {\n          altText\n          mediaDetails {\n            sizes(include: $include) {\n              height\n              name\n              width\n              sourceUrl\n            }\n          }\n          srcSet(size: $size)\n\t\t  sizes(size: $size)\n        }\n      }\n      title(format: RENDERED)\n      link\n\t  eventId\n      eventTypes {\n        nodes {\n          event_typeId\n          name\n          link\n        }\n      }\n    }\n  }\n  eventTypes {\n    nodes {\n      event_typeId\n      name\n\t  link\n    }\n  }\n  eventLocations {\n    nodes {\n      name\n      link\n      event_locationId\n    }\n  }\n}',
				variables: {
					first: Number( n ),
					after: '',
					include: [ 'LARGE' ],
	...
	function a( t ) {
		let { filters: n, checkedFilters: s, setCheckedFilters: r } = t;
		return ( 0, e.createElement )(
			'div',
			{ className: 'cno-event-search-filters' },
			n.map( ( t ) => {
				let {
					type: { name: n, filters: i },
				} = t;
				return ( 0, e.createElement )(
					'div',
					{
	...
	function o( t ) {
		let { search: n, handleSearchInput: s } = t;
		return ( 0, e.createElement )( 'input', {
			type: 'text',
			name: 'search',
			id: 'search',
	...
	var h = new ( class {
		getSchedule() {
			...
			return null === t ? { friday: [], saturday: [], sunday: [] } : t;
		}
		addToSchedule( e ) {
			let { target: t } = e;
			return new Promise( ( e, n ) => {
				try {
					this.checkTargetElement( t );
					const s = Number( t.dataset.id ),
						r = this.getSchedule();
					try {
						this.getEventData( s ).then( ( t ) => {
							const n = t.day.toLowerCase(),
								s = r[ n ].filter( ( e ) => e.id === t.id );
							0 === s.length
								? ( r[ n ].push( t ),
		...
		getEventData = async ( e ) => {
			try {
				const t = await fetch(
						`${ cnoSiteData.rootUrl }/wp-json/wp/v2/events/${ e }?_fields=acf,title,link`
					),
					n = await t.json(),
					{
						acf: { info: s },
	...
	function d( t ) {
		...
		return (
			...
			( 0, e.createElement )(
				...
				( 0, e.createElement )(
					'a',
					{ href: s, className: 'btn__outline--primary' },
					'Learn More'
				),
		...
		return ( 0, e.createElement )(
			...
			( 0, e.createElement )(
				...
				( 0, e.createElement )(
					...
					( 0, e.createElement )( 'strong', null, 'When: ' ),
					a.info.day,
					', September 1'
				),
				s.length > 0 &&
					( 0, e.createElement )(
						...
						( 0, e.createElement )( 'strong', null, 'Where:' ),
						' ',
						( 0, e.createElement )(
							'a',
							{ href: s[ 0 ].link, rel: 'tag' },
							s[ 0 ].name
						)
					),
				( 0, e.createElement )(
					...
					( 0, e.createElement )( 'strong', null, 'Start Time:' ),
					' ' + a.info.startTime
				),
				a.info.endTime &&
					( 0, e.createElement )(
						...
						( 0, e.createElement )( 'strong', null, 'End Time:' ),
						' ',
						a.info.endTime
					),
				( 0, e.createElement )(
					...
					( 0, e.createElement )( 'strong', null, 'Event Type:' ),
					' ',
					( 0, e.createElement )(
						'a',
						{ href: '${type[0].link}', rel: 'tag' },
						m[ 0 ].name
					)
				)
			),
			...
			( 0, e.createElement )( d, { eventId: r, link: i } )
		);
	}
	function m( t ) {
		let { posts: n, checkedFilters: s } = t;
		return ( 0, e.createElement )(
			'section',
			{ className: 'cno-events' },
			n.map( ( t ) => {
				if ( 0 === s.length )
					return ( 0, e.createElement )( u, { data: t } );
				if (
					[ s, t.locations, t.type ].every( ( e ) => e.length > 0 )
				) {
					if (
						s.includes( t.locations?.[ 0 ]?.name ) ||
						s.includes( t.type[ 0 ]?.name )
					)
						return ( 0, e.createElement )( u, { data: t } );
				} else if (
					s.includes( t.locations?.[ 0 ]?.name ) &&
	...
	const _ = ( e ) => `Missing ${ e } property in key`,
		x = ( e ) =>
	...
	function w( e ) {
		let t = null,
			...
			r = 1,
			i = null;
		if ( g( e ) || f( e ) ) ( s = e ), ( t = L( e ) ), ( n = I( e ) );
		else {
			if ( ! k.call( e, 'name' ) ) throw new Error( _( 'name' ) );
			const c = e.name;
			if (
				( ( s = c ),
				k.call( e, 'weight' ) && ( ( r = e.weight ), r <= 0 ) )
			)
				throw new Error( x( c ) );
			( t = L( c ) ), ( n = I( c ) ), ( i = e.getFn );
		}
		return { path: t, id: n, weight: r, src: s, getFn: i };
	}
	function L( e ) {
				...
				if ( v( e ) )
					if ( t[ i ] ) {
						...
						if (
							...
							s = ! 0;
							for ( let e = 0, n = c.length; e < n; e += 1 )
								r( c[ e ], t, i + 1 );
						} else t.length && r( c, t, i + 1 );
					} else n.push( e );
			};
			return r( e, g( t ) ? t.split( '.' ) : t, 0 ), s ? n : n[ 0 ];
		},
		ignoreLocation: ! 1,
		ignoreFieldNorm: ! 1,
		fieldNormWeight: 1,
	...
	class C {
		constructor( {
			getFn: e = N.getFn,
			fieldNormWeight: t = N.fieldNormWeight,
		} = {} ) {
			( this.norm = ( function ( e = 1, t = 3 ) {
				...
				return {
					get( t ) {
						const r = t.match( $ ).length;
						if ( n.has( r ) ) return n.get( r );
						const i = 1 / Math.pow( r, 0.5 * e ),
							c = parseFloat( Math.round( i * s ) / s );
						return n.set( r, c ), c;
					},
					clear() {
						n.clear();
	...
	function b(
		...
		const r = new C( { getFn: n, fieldNormWeight: s } );
		return r.setKeys( e.map( w ) ), r.setSources( t ), r.create(), r;
	}
	function F(
		e,
		{
	...
	class T {
		constructor(
			e,
			{
				...
				findAllMatches: i = N.findAllMatches,
			...
			if ( h > A ) {
				...
				const t = h % A,
					n = h - t;
				for ( ; e < n;  )
					l( this.pattern.substr( e, A ), e ), ( e += A );
				if ( t ) {
					const e = h - A;
		...
		searchIn( e ) {
			...
			let l = [],
				...
				d = ! 1;
			this.chunks.forEach(
				( { pattern: t, alphabet: u, startIndex: m } ) => {
					const {
						isMatch: f,
						score: g,
						indices: p,
						...
						for ( let s = 0; s < h; s += 1 ) {
							...
							for ( ; i < a;  )
								F( t, {
									...
									? ( i = a )
									: ( M = a ),
									( a = Math.floor( ( M - i ) / 2 + i ) );
							M = a;
							let o = Math.max( 1, u - a + 1 ),
								y = c ? d : Math.min( u + a, d ) + h,
								x = Array( y + 2 );
							x[ y + 1 ] = ( 1 << s ) - 1;
							for ( let i = y; i >= o; i -= 1 ) {
								let c = i - 1,
						...
						const x = {
							isMatch: f >= 0,
							score: Math.max( 0.001, E ),
						};
						if ( g ) {
							const e = ( function (
								e = [],
								t = N.minMatchCharLength
							) {
								let n = [],
									s = -1,
									r = -1,
	...
	const P = [
		...
		q = / +(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)/,
		D = new Set( [ j.type, W.type ] );
	const J = [];
	function K( e, t ) {
		for ( let n = 0, s = J.length; n < s; n += 1 ) {
			let s = J[ n ];
	...
	const G = '$and',
		...
		B = ( e ) => ! ( ! e[ G ] && ! e.$or ),
		H = ( e ) => ( {
			[ G ]: Object.keys( e ).map( ( t ) => ( { [ t ]: e[ t ] } ) ),
		} );
	function V( e, t, { auto: n = ! 0 } = {} ) {
		const s = ( e ) => {
			let r = Object.keys( e );
			const i = ( ( e ) => !! e[ U ] )( e );
			if ( ! i && r.length > 1 && ! B( e ) ) return s( H( e ) );
			if ( ( ( e ) => ! f( e ) && y( e ) && ! B( e ) )( e ) ) {
				const s = i ? e[ U ] : r[ 0 ],
					c = i ? e.$val : e[ s ];
				if ( ! g( c ) )
					throw new Error(
						( ( e ) => `Invalid value for key ${ e }` )( s )
					);
				const a = { keyId: I( s ), pattern: c };
				return n && ( a.searcher = K( c, t ) ), a;
			}
	...
	class Y {
		...
		remove( e = () => ! 1 ) {
			...
			for ( let n = 0, s = this._docs.length; n < s; n += 1 ) {
				...
				e( r, n ) &&
					( this.removeAt( n ), ( n -= 1 ), ( s -= 1 ), t.push( r ) );
			}
			return t;
		}
		removeAt( e ) {
		...
		search( e, { limit: t = -1 } = {} ) {
			...
			return (
				( function ( e, { ignoreFieldNorm: t = N.ignoreFieldNorm } ) {
					e.forEach( ( e ) => {
						let n = 1;
						e.matches.forEach(
							( { key: e, norm: s, score: r } ) => {
				...
				n.forEach( ( { v: e, i: n, n: r } ) => {
					...
					i &&
						s.push( {
							...
							idx: n,
							matches: [
								{ score: c, value: e, norm: r, indices: a },
							],
						} );
				} ),
				s
			...
			const t = V( e, this.options ),
				n = ( e, t, s ) => {
					if ( ! e.children ) {
						...
						return i && i.length
							? [ { idx: s, item: t, matches: i } ]
							: [];
					}
					const r = [];
					for ( let i = 0, c = e.children.length; i < c; i += 1 ) {
						const c = e.children[ i ],
							a = n( c, t, s );
						if ( a.length ) r.push( ...a );
						else if ( e.operator === G ) return [];
					}
					return r;
				},
			...
			return (
				s.forEach( ( { $: e, i: s } ) => {
					...
					n.forEach( ( n, s ) => {
						...
						i.length && r.push( { idx: s, item: e, matches: i } );
				} ),
				r
			);
		}
		_findMatches( { key: e, value: t, searcher: n } ) {
			...
			if ( f( t ) )
				...
				const { v: r, n: i } = t,
					{ isMatch: c, score: a, indices: o } = n.searchIn( r );
				c &&
					s.push( {
						score: a,
						key: e,
	...
	( Y.version = '6.6.2' ),
		...
		( Y.parseIndex = function (
			e,
			{ getFn: t = N.getFn, fieldNormWeight: n = N.fieldNormWeight } = {}
		) {
			const { keys: s, records: r } = e,
				i = new C( { getFn: t, fieldNormWeight: n } );
			return i.setKeys( s ), i.setIndexRecords( r ), i;
		} ),
		( Y.config = N ),
		( Y.parseQuery = V ),
		( function ( ...e ) {
			...
			class {
				...
				searchIn( e ) {
					...
					const { includeMatches: n, isCaseSensitive: s } =
						this.options;
					e = s ? e : e.toLowerCase();
					let r = 0,
						i = [],
	...
	Z &&
		( 0, e.createRoot )( Z ).render(
			( 0, e.createElement )( function () {
				const [ n, s ] = ( 0, e.useState )( ! 0 ),
					[ r, a ] = ( 0, e.useState )( [] ),
					[ o, h ] = ( 0, e.useState )( [] ),
					[ d, u ] = ( 0, e.useState )( '' );
				( 0, e.useEffect )( () => {
					...
					( 0, e.useEffect )( () => {
						...
						s( ! 0 );
						const e = setTimeout( () => {
							const e = new Y( r, { ...i } ).search( d );
							a( e.map( ( e ) => e.item ) ), s( ! 1 );
						}, 500 );
						return () => clearTimeout( e );
					}, [ d ] );
				const [ f, g ] = ( 0, e.useState )( [] );
				return ( 0, e.createElement )(
					'div',
