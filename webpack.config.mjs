 ...
 */
const appNames = ['front-page', 'my-schedule']; // for page-specific js files
	...
	return {
		...
		entry: async () => {
			return {
				...
				'vendors/animate': path.resolve(
					THEME_SRC,
					'styles/vendors/animate.min.css'
				),
				'pages/map': path.resolve( THEME_SRC, 'js/map/MapController.ts' ),
				'pages/registrations': path.resolve(
					THEME_SRC,
					'js/registrations/RegistrationsHandler.ts'
				),
				'modules/add-to-schedule': path.resolve(
					THEME_SRC,
					'js/add-to-schedule/controller.ts'
				),
